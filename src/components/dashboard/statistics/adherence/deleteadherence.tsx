...
import { delete_adherence } from "../../../../lib/fetch/statistics/log-adherence/adherence";
...
export default function DeleteAdherence({ adherence_id }: Props) {
  const [isLoading, setIsLoading] = useState(false);
  const [openWarning, setOpenWarning] = useState(false);

  const router = useRouter();
  async function deleteProgress(adherence_id: number) {
    setIsLoading(true);
    try {
      const access_token = Cookies.get("access_token") ?? "";

      await delete_adherence({
        access_token,
        adherence_id,
      });

      toast({
        message: "adherence deleted",
        type: "success",
      });
